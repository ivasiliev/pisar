...
class DocOfficialProceedingProtocol(DocumentInReport):
	...
	def render(self):
		...
		self.add_paragraph(sold.full_name, self.align_justify_underline)
		...
		self.add_paragraph(f"Рапорт {commander_company}, объяснения сослуживцев, другие материалы служебного разбирательства", self.align_justify_underline)
		...
		self.render_combined_row("Подпись военнослужащего: ", f"нет, в связи с отсутствием {sold.full_name}")
		self.render_combined_row("Объяснения военнослужащего, совершившего грубый дисциплинарный проступок: ", f"нет, в связи с отсутствием {sold.full_name}")
		self.render_combined_row("Смягчающие или отягчающие обстоятельства: ", "нет")
		self.render_combined_row("Причины и условия, способствовавшие совершению грубого дисциплинарного проступка: ", f"низкие морально-деловые качества {sold.full_name}")
		...
		self.render_combined_row("Копию протокола получил: ",  f"нет, в связи с отсутствием {sold.full_name}")
